public class Computer {
            ...
            //01xx 
                ...
                if(currentInstruction.getBit(2).getValue()){
                    if(currentInstruction.getBit(3).getValue()){  
                        ...
                        //Stack Commands 0110  - Execute
                        if(currentInstruction.getBit(4).getValue()){
                            if(currentInstruction.getBit(5).getValue() == false){
                            //10 - Call
                            //write the next instruction into memory
                            internalMemory.write(SP, programCounter);
                            }
                            //11 - Return
                            else{
                                SP = rippleAdder.add(SP, new Longword(4));
                                op1 = internalMemory.read(SP);
                            }
                        } 
                        else{
                            //01 - Pop
                            if(currentInstruction.getBit(5).getValue()){
                                SP = rippleAdder.add(SP, new Longword(4));
                                //op1.copy(currentInstruction.rightShift(16).and(new Longword(15)));
                                System.out.println();
                            }
                            //00 - Push 
                            else{
                                //op1.copy(computerRegisters[currentInstruction.rightShift(16).and(new Longword(15)).getSigned()]);
                            }
                        }
                    }
                }
    ...
    private void store(){
        ...
        if(currentInstruction.getBit(0).getValue() == true){
            computerRegisters[(int) currentInstruction.rightShift(16).and(new Longword(15)).getUnsigned()].copy(resultant);
        }
        //0xxx
        else{
            if(currentInstruction.getBit(1).getValue() == true){//01xx
                if(currentInstruction.getBit(2).getValue() == true){//011x
                    if(currentInstruction.getBit(3).getValue() == true){//0111
                        ...
                        if(currentInstruction.getBit(4).getValue()){
                            if(currentInstruction.getBit(5).getValue() == false){
                                //Stack Commands - Store function
                                //0110 10 - Call
                                    //Jump portion
                                    programCounter.copy(currentInstruction.rightShift(16).and(new Longword(1023)));
                                    //add next address to memory
                                    SP = rippleAdder.subtract(SP, new Longword(4));
                                }
                                //0110 11 - Return 
                                else{
                                    //SP increment happens in execute
                                    programCounter.copy(op1);
                                }
                        } 
                        else{
                            //0110 00 - Push 
                            if(currentInstruction.getBit(5).getValue() == false){
                                //write that data from the register into stack
                                internalMemory.write(SP, computerRegisters[(int) currentInstruction.rightShift(16).and(new Longword(15)).getUnsigned()]);
                                //Decrement SP
                                SP = rippleAdder.subtract(SP, new Longword(4));
                                System.out.println();
                            }
                            //0110 01 - Pop
                                ...
                                computerRegisters[(int) currentInstruction.rightShift(16).and(new Longword(15)).getUnsigned()].copy(internalMemory.read(SP));
                            }
                        }
                    }
                }
    ...
    public int getPCValue(){
        return programCounter.getSigned();
    }
